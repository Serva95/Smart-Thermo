...
public class TermoManagement {
    ...
    public static void insert(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            request.setAttribute("viewUrl", "Connector?ca=TermoManagement.view");
        } catch (Exception e) {
            try {
                if (daoFactory != null) {
                    daoFactory.rollbackTransaction();
    ...
    public static void editRoom(HttpServletRequest request, HttpServletResponse response) {
        SessionDAOFactory sessionDAOFactory;
        LoggedUser loggedUser;
        Utente user;
        DAOFactory daoFactory = null;
        try {
            ...
            request.setAttribute("loggedOn",loggedUser!=null);
            request.setAttribute("loggedUser", loggedUser);
            request.setAttribute("viewUrl", "stanzaEdit");
        } catch (Exception e) {
            //logger.log(Level.SEVERE, "Controller Error", e);
            try {
                if (daoFactory != null) {
    ...
    public static void update(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            stanza = new Stanza(name, maxTemp, minTemp, absMin, id);
            LocalTime[][] giorniOnOff = new LocalTime[7][];
            for(int i=0; i<7; i++) {
                LocalTime[] onOff = new LocalTime[6];
                onOff[0] = LocalTime.parse(request.getParameter("timeonuno"+i));
